...
function renderUserListLegacy(){ $('#userList').innerHTML=Object.entries(users).filter(([,user])=>user.role==='user').map(([,user])=>`<span class="party-chip">${escapeHtml(user.name)} <small>${escapeHtml(user.username)} · ${escapeHtml(user.location)}</small></span>`).join('');setAdminControls(); }
...
async function saveUserForm(){const name=$('#userNameInput').value,username=$('#userLoginIdInput').value,locationId=$('#userLocationSelect').value;if(state.editingUserId){await api(`/api/users/${encodeURIComponent(state.editingUserId)}`,{method:'PATCH',body:{name,username,locationId}});await loadWorkspace();toast('User updated');resetUserEditor();return;}await addUser(name,username,locationId,$('#userPasswordInput').value);resetUserEditor();}
...
async function saveEntry(formElement){
 ...
 if(state.editingEntry){const data=await api(`/api/entries/${encodeURIComponent(state.editingEntry.id)}`,{method:'PATCH',body:{...body,adminEdit:true}});replaceEntry(data.entry);await loadWorkspace();closeModal();toast('Entry updated');return;}
 if(direction==='outward'&&isMaterial&&!masters.customers.length)throw new Error('Add a customer name in Settings before creating a material outward entry.');
 const data=await api('/api/entries',{method:'POST',body});replaceEntry(data.entry);refreshWorkspace();closeModal();if(direction==='inward'&&isMaterial)inwardNotification(`Token ${data.entry.token}: inward entry created with Pending status`);else toast(direction==='outward'?`Exit recorded with token ${data.entry.outwardToken}`:'Entry saved and visitor pass generated');
}
async function restoreSession(){try{await loadWorkspace();$('#loginScreen').hidden=true;$('#appShell').hidden=false;}catch(error){if(error.status!==401)$('#loginError').textContent='The service is unavailable. Please try again.';showLogin(false);}}
